...
if(isPost()) {
   ...
   $filterFull = '';
   if($full == 0) {
       $filterFull = 'AND rp.status != 1 AND rp.status != 5';
   }
   $nameFactory = "";
   ...
   if(!empty($object)) {
      ...
      if($status != 3) {
         if($object != 'all') {
            $sqlTotalInspect = "SELECT SUM(quantity_inspect) as total_inspect FROM reports AS rp $filter $filterFull AND factory_id = $object";
            ...
            $filterTotal $filterFull AND rd.level = 'Nghiêm trọng'
            GROUP BY df.cate_id;";
            ...
            JOIN reports AS rp ON rp.id = rd.report_id 
            JOIN defects AS df ON df.id = rd.defect_id
            $filterTotal $filterFull AND rd.level = 'Nặng'
            GROUP BY df.cate_id;";
            ...
            JOIN reports AS rp ON rp.id = rd.report_id 
            JOIN defects AS df ON df.id = rd.defect_id
            $filterTotal $filterFull AND rd.level = 'Nhẹ'
            GROUP BY df.cate_id;";
            ...
            foreach($listAllCates as $item) {
               ...
               $dataTableExcel["data"][] = [
                  ...
                  $item['name'],
                  $totalInspectValue,
                  (!empty($item['total_defect']) ? $item['total_defect'] : 0),
                  (!empty($item['total_serious']) ? $item['total_serious'] : 0),
                  (!empty($item['total_heavy']) ? $item['total_heavy'] : 0),
                  (!empty($item['total_light']) ? $item['total_light'] : 0),
                  (!empty($item['percent_serious']) ? $item['percent_serious']."%" : "0%"),
                  (!empty($item['percent_heavy']) ? $item['percent_heavy']."%" : "0%"),
                  (!empty($item['percent_light']) ? $item['percent_light']."%" : "0%"),
                  (!empty($item['percent']) ? $item['percent']."%" : "0%")
               ];
            }
            ...
            $config = '
            {
               ...
               "options": {
                  ...
                  "plugins": {
                     "title": {
                        "display": true,
                        "text": "BIỂU ĐỒ TỶ LỆ LỖI CHẤT LƯỢNG MAY ĐẦU VÀO CỦA CƠ SỞ '.mb_strtoupper($nameFactory, 'UTF-8').' '.$time.'"
                     }
                  },
               "scales": {
                  "y": {
            ...
            ';
            $dataTableExcel['title'] = 'BIỂU ĐỒ TỶ LỆ LỖI CHẤT LƯỢNG MAY ĐẦU VÀO CỦA CƠ SỞ '.mb_strtoupper($nameFactory, 'UTF-8')." $time";
         } else {
            ...
            $sqlQuantityReport = "SELECT factory_id, COUNT(id) AS quantity_report FROM reports AS rp $filter $filterFull GROUP BY factory_id;";
            if($skip == 1) {
               $sqlQuantityReportSerious = "SELECT rp.factory_id, COUNT(rp.ID) AS quantity_report_serious FROM reports AS rp JOIN resultaql AS RA ON RA.report_id = rp.ID $filter $filterFull AND RA.quantity_serious_real > 0 GROUP BY rp.factory_id;";
            } else {
            ...
            foreach($listAllFactories as $item) {
               ...
               if($isMonth) {
                  ...
                  $dataTableExcel["data"][] = [
                     ...
                     $item['total_inspect'],
                     $item['total_defect'],
                     $item['quantity_report_serious'],
                     $item['quantity_report'],
                     $item['total_serious'],
                     $item['total_heavy'],
                     $item['total_light'],
                     $item['percent_serious'].'%',
                     $item['percent_heavy'].'%',
                     $item['percent_light'].'%',
                     $item['percent'].'%',
                     $item['score'],
            ...
            $config = '
            {
               ...
               "options": {
                  ...
                  "plugins": {
                     "title": {
                        "display": true,
                        "text": "BIỂU ĐỒ TỈ LỆ LỖI CHẤT LƯỢNG MAY ĐẦU VÀO '.$time.'"
                     }
                  },
               "scales": {
                  "y": {
            ...
            ';
            $dataTableExcel['title'] = "BIỂU ĐỒ TỈ LỆ LỖI CHẤT LƯỢNG MAY ĐẦU VÀO $time";
         }
      } else {
         if($object == 'all') {
            if($skip == 1) {
               $sql = "SELECT
                  ...
                  $filter $filterFull AND MONTH(ra.create_at) BETWEEN 1 AND 12
                  GROUP BY
                  MONTH(ra.create_at)";
            } else {
               $sqlSerious = "SELECT
               ...
               reports AS rp
               JOIN report_defect AS rd ON rd.report_id = rp.id AND rd.level = 'Nghiêm trọng'
               $filter $filterFull AND MONTH(rp.create_at) BETWEEN 1 AND 12
               GROUP BY
               MONTH(rp.create_at);";
            ...
            if($skip == 1) {
               ...
               JOIN
               reports AS rp ON rp.id = ra.report_id
               $filter $filterFull AND MONTH(ra.create_at) BETWEEN 1 AND 12 AND rp.factory_id = $object AND ra.quantity_serious_real > 0
               GROUP BY
               MONTH(ra.create_at)";
            } else {
               $sqlSerious = "SELECT
               ...
               reports AS rp
               JOIN report_defect AS rd ON rd.report_id = rp.id AND rd.level = 'Nghiêm trọng'
               $filter $filterFull AND MONTH(rp.create_at) BETWEEN 1 AND 12 AND rp.factory_id = $object
               GROUP BY
               MONTH(rp.create_at);";
               ...
               FROM reports AS rp
               JOIN report_defect AS rd ON rd.report_id = rp.id AND rd.level = 'Nghiêm trọng'
               $filter $filterFull AND MONTH(rp.create_at) BETWEEN 1 AND 12 AND factory_id = $object
               GROUP BY MONTH(rp.create_at);";
            }
            $isNotAll = true;
         }
         ...
         if($isNotAll) {
            ...
            foreach($listMonth as $item) {
               ...
               if($isNotAll) {
                  ...
                  $dataTableExcel['data'][] = [
                     ...
                     $item['total_inspect'],
                     $item['quantity_report'],
                     (!empty($item['quantity_report_serious']) ? $item['quantity_report_serious'] : 0),
                     $item['total_serious'],
                     $item['total_heavy'],
                     $item['total_light'],
                     $item['percent_serious'].'%',
                  ...
                  <tr>
                     <td> Tháng '.$item['month'].'</td>
                     <td>'.$item['total_inspect'].'</td>
                     <td>'.($item['total_serious'] + $item['total_heavy'] + $item['total_light']).'</td>
                     <td>'.$item['total_serious'].'</td>
                     <td>'.$item['total_heavy'].'</td>
                     <td>'.$item['total_light'].'</td>
                     <td>'.$item['percent_serious'].'%</td>
                  ...
                  $dataTableExcel['data'][] = [
                     'Tháng '.$item['month'],
                     $item['total_inspect'],
                     ($item['total_serious'] + $item['total_heavy'] + $item['total_light']),
                     $item['total_serious'],
                     $item['total_heavy'],
                     $item['total_light'],
                     $item['percent_serious'].'%',
            ...
            ';
            $dataTableExcel['title'] = "BIỂU ĐỒ TỶ LỆ LỖI CHẤT LƯỢNG MAY ĐẦU VÀO THEO 12 THÁNG $time";
      }
   }
...
$data = [
   'dataTable' => $dataTable,
   'dataChart' => $dataChart,
   'dataTableExcel' => json_encode($dataTableExcel)
];
