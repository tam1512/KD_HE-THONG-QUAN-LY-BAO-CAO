...
if(isPost()) {
   ...
   if(!empty($object)) {
      ...
      if($object != 'all') {
         ...
         $sqlTotalSeriuos = "SELECT df.cate_id, SUM(rd.defect_quantity) 
         FROM report_defect AS rd 
         JOIN reports AS rp ON rp.id = rd.report_id 
         JOIN defects AS df ON df.id = rd.defect_id
         ...
         $sqlFirst = "SELECT f.id, f.name, SUM(quantity_deliver) AS total_deliver, SUM(quantity_inspect) AS total_inspect
         FROM reports AS rp 
                  ...
                  if($first["id"] == $listAllFactoriesSecond[$key]['factory_id']) {
                     $dataAllFactoties[] = [
                        ...
                        "total_inspect" => $first["total_inspect"],
                         ...
                         "percent_serious" => $listAllFactoriesSecond[$key]["total_serious"] / $first["total_inspect"] * 100,
                         "percent_heavy" => $listAllFactoriesSecond[$key]["total_heavy"] / $first["total_inspect"] * 100,
                         "percent_light" => $listAllFactoriesSecond[$key]["total_light"] / $first["total_inspect"] * 100,
                     ];
                  }
            }
         }
