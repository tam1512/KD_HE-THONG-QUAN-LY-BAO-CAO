      ...
      if($status != 3) {
         if($object != 'all') {
            ...
            if($kip == 1) {
               ...
               $sqlSerious = "SELECT rp.factory_id, SUM(rd.defect_quantity) AS quantity_serious FROM reports AS rp
               JOIN report_defect AS rd ON rp.id = rd.report_id
               JOIN defects AS df ON rd.defect_id = df.id
               $filter AND rd.level = 'Nghiêm trọng'
               GROUP BY rp.factory_id;";
               $sqlHeavy = "SELECT rp.factory_id, SUM(rd.defect_quantity) AS quantity_heavy FROM reports AS rp
               JOIN report_defect AS rd ON rp.id = rd.report_id
               JOIN defects AS df ON rd.defect_id = df.id
               $filter AND rd.level = 'Nặng'
               GROUP BY rp.factory_id;";
               $sqlLight = "SELECT rp.factory_id, SUM(rd.defect_quantity) AS quantity_light FROM reports AS rp
               JOIN report_defect AS rd ON rp.id = rd.report_id
               JOIN defects AS df ON rd.defect_id = df.id
               $filter AND rd.level = 'Nhẹ'
               GROUP BY rp.factory_id;";
            }
