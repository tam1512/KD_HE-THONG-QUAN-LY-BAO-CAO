      ...
      if($status != 3) {
            ...
            if(!empty($listAllCates)) {
               foreach($listAllCates as $key => $cate) {
                  ...
                  if(!empty($listTotalLight)) {
                     $cate["total_light"] = $listTotalLight[$key]['total_light'];
                  } else {
                     $cate["total_light"] = 0;
                  }
         ...
         foreach($listMonth as $key => $m) {
            if(!empty($listMonthArr[$key])) {
               ...
               $m['total_serious'] = round($listMonthArr[$key]['total_serious'] / $listMonthArr[$key]['total_inspect'] * 100, 2);
               $m['total_heavy'] = round($listMonthArr[$key]['total_heavy'] / $listMonthArr[$key]['total_inspect'] * 100, 2);
               $m['total_light'] = round($listMonthArr[$key]['total_light'] / $listMonthArr[$key]['total_inspect'] * 100, 2);
            } else {
               $m['total_serious'] = 0;
               $m['total_heavy'] = 0;
               $m['total_light'] = 0;
