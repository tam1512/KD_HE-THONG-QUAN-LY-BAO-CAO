   ...
   if(!empty($object)) {
      ...
      if($object != 'all') {
         ...
         foreach($listAllCates as $key=>$item) {
            ...
            $percentSerious = ($item['total_serious']) / $total * 100;
            $percentHeavy = ($item['total_heavy']) / $total * 100;
            $percentLight = ($item['total_light']) / $total * 100;
            ...
            foreach($listAllFactoriesFirst as $key => $first) {
                  if($first["id"] == $listAllFactoriesSecond[$key]['factory_id']) {
                     $dataAllFactoties[] = [
                        ...
                        "percent" => round($listAllFactoriesSecond[$key]["total_defect"] / $first["total_inspect"] * 100, 2)
                     ];
                  }
            }
         }
