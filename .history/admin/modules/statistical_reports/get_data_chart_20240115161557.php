 ...
 * - all: giống theo tháng, NGHIÊM TRỌNG TÍNH THEO SỐ BIÊN BẢN
   ...
   if(!empty($object)) {
      ...
      if($status != 3) {
         if($object != 'all') {
            ...
            $sqlSecond = "SELECT rp.factory_id, SUM(quantity_heavy_real) AS total_heavy, SUM(quantity_light_real) AS total_light 
            FROM resultaql AS ra JOIN reports AS rp ON rp.id = ra.report_id 
            $filter
            GROUP BY rp.factory_id;";
            
            $sqlQuantityReport = "SELECT factory_id, COUNT(id) AS quantity_report FROM REPORTS $filter GROUP BY factory_id;";
            $sqlQuantityReportSerious = "SELECT RP.factory_id, COUNT(RP.ID) AS quantity_report_serious FROM reports AS RP JOIN resultaql AS RA ON RA.report_id = RP.ID WHERE YEAR(RP.create_at) = 2024 AND RA.quantity_serious_real > 0 GROUP BY RP.factory_id;";
            $labels = "";
            $dataTotalInspect = "";
            $dataTotalDefect = "";
            $dataTotalSerious = "";
            ...
            if(!empty($listQuantityReport)) {
               foreach($listQuantityReport as $key => $quantityReport) {
                  if(!empty($listQuantityReport)) {
                     foreach($listQuantityReportSerious as $qRS) {
                        if($quantityReport["factory_id"] == $qRS["factory_id"]) {
                           $quantityReport['quantity_report_serious'] = $qRS["quantity_report_serious"];
                           $listQuantityReport[$key] = $quantityReport;
                        }
                     }
                  } else {
                     $quantityReport['quantity_report_serious'] = 0;
                     $listQuantityReport[$key] = $quantityReport;
                  }
               }
            }
               ...
               foreach($listAllFactoriesFirst as $key => $first) {
                     if($first["id"] == $listAllFactoriesSecond[$key]['factory_id']) {
                        $dataAllFactoties[] = [
                           ...
                           "total_serious" => $listQuantityReport[$key]["quantity_report_serious"],
                           "total_heavy" => $listAllFactoriesSecond[$key]["total_heavy"],
                           "total_light" => $listAllFactoriesSecond[$key]["total_light"],
                           "percent_serious" => round($listQuantityReport[$key]["quantity_report_serious"] / $listQuantityReport[$key]["quantity_report"] * 100, 2),
