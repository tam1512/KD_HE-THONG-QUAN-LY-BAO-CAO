...
if($isAll) {
   ...
   foreach($listAllFactories as $key=>$fac) {
      foreach($dataChart as $item) {
         if($fac['id'] == $item['id']) {
            if(!empty($item['total_deliver'])) {
               $fac['total_deliver'] = $item['total_deliver'];
            } else {
               $fac['total_deliver'] = 0;
            }
            if(!empty($item['total_inspect'])) {
               $fac['total_inspect'] = $item['total_inspect'];
            }else {
               $fac['total_inspect'] = 0;
            }
            if(!empty($item['total_defect'])) {
               $fac['total_inspect'] = $item['total_defect'];
            }else {
               $fac['total_inspect'] = 0;
            }

            if(!empty($item['total_defect']) && !empty($item['total_inspect'])) {
