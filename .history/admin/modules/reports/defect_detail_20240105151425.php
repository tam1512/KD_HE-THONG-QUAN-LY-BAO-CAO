...
$report = firstRaw("SELECT rp.po_code, rp.code_report, rp.defect_finder, rp.quantity_deliver, rp.quantity_inspect, rp.comment, rp.suggest, rp.user_id, rp.create_at, rs.userXX, rs.userQD, rs.userPD, f.name AS factory_name, p.name AS product_name, rp.create_at FROM reports AS rp JOIN factories AS f ON f.id = rp.factory_id JOIN products AS p ON p.id = rp.product_id JOIN report_sign AS rs ON report_id = rp.id WHERE rp.id = $reportId");

$sign_userKT = firstRaw("SELECT sign_text FROM sign WHERE user_id = ". $report["user_id"])['sign_text'];
$fullnameUserKT = firstRaw("SELECT fullname FROM users WHERE user_id = ". $report["user_id"])['fullname'];
...
$fullnameUserXX = firstRaw("SELECT fullname FROM users WHERE user_id = ". $userXX->user_id)['fullname'];
...
$fullnameUserQD = firstRaw("SELECT fullname FROM users WHERE user_id = ". $userQD->user_id)['fullname'];
...
$fullnameUserPD = firstRaw("SELECT fullname FROM users WHERE user_id = ". $userPD->user_id)['fullname'];
...
<div class="container content-pdf">
   ...
   <table class="table">
      <tr>
         <td width="20%"><b>Người kiểm tra</b></td>
         <td><b>Công sở gia công</b></td>
         <td><b>Người xem xét</b></td>
         <td><b>QĐ/PQĐ</b></td>
      ...
      <tr>
         <td>
            <?php echo !empty($sign_userKT) ? '<img src="'.$sign_userKT.'" alt="" class="sign"><p class=""></p><p class="d-block">'.getDateFormat($report['create_at'], 'd-m-Y H:i:s').'</p>' : false ?>
         </td>
         <td>
            <div id="sign_CSGC"></div>
            <input type="hidden" name="sign_CSGC" id="sign_CSGC_value" value="">
         </td>
         <td>
            <?php echo !empty($sign_userXX) && $status_userXX == 1 ? '<img src="'.$sign_userXX.'" alt="" class="sign">' : false ?>
         </td>
         <td>
            <?php echo !empty($sign_userQD) && $status_userQD == 1 ? '<img src="'.$sign_userQD.'" alt="" class="sign">' : false ?>
         </td>
         <td>
            <?php echo !empty($sign_userPD) && $status_userPD == 1 ? '<img src="'.$sign_userPD.'" alt="" class="sign">' : false ?>
         </td>
      </tr>
   </table>
   <br>
