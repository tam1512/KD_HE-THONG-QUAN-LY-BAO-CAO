      ...
      if(!empty($listAllReportDefectFinal)) {
         foreach($listAllReportDefectFinal as $itemUpdate) {
            ...
            if($statusUpdateReportDefect){
               if(!empty($itemUpdate['files'])) {
                  $listImageUpdate = $itemUpdate['files'];
                  echo '<pre>';
                  print_r($listImageUpdate);
                  echo '</pre>';
                  $reportDefectId = firstRaw("SELECT id FROM report_defect WHERE report_id = $reportId AND defect_id = ".$itemUpdate['defect_id'])['id'];

                  $listImageOldQuery = getRaw("SELECT link FROM report_defect_images WHERE report_defect_id = $reportDefectId");

                  if(!empty($listImageOldQuery)) {
                     $statusDeleteImageRD = delete('report_defect_images', "report_defect_id = $reportDefectId");  
                  }

                  //đang không upload được hình ảnh
                  foreach($listImageUpdate as $file) {
                     $dataInsertImage = [
                        'image_link' => $file,
                        'report_defect_id' => $reportDefectId,
                        'create_at' => date('Y-m-d H:i:s')
                  ...
                  <table class="table table-bordered">
                     ...
                     ?>
                        <tr>
                           ...
                           <td>
                              <input type="number" name="<?php echo $key ?>" class="form-control"
