 ...
 if(isPost()) {
   ...
   if(empty($errors)) {
      ...
      if($userXX->user_id == $userXXId) {
         ...
         $dataUpdateReportSign["userXX"] = '{"user_id":'.$userXXId.', "status":0}';
      }
      if($userQD->user_id == $userQDId) {
         $dataUpdateReportSign["userQD"] = json_encode($userQD);
      } else {
         $dataUpdateReportSign["userQD"] = '{"user_id":'.$userQDId.', "status":0}';
      }
      if($userPD->user_id == $userPDId) {
         $dataUpdateReportSign["userPD"] = json_encode($userPD);
      } else {
         $dataUpdateReportSign["userPD"] = '{"user_id":'.$userPDId.', "status":0}';
      }
