<?php 
   $id = !empty(isLogin()['user_id']) ? isLogin()['user_id'] : false;
   if(!empty($id)) {
...
<div class="notification-list">
   ...
   <a href="<?php echo getLinkAdmin('reports', 'seen', ['id' => $noti['report_id']]).'?seen=1' ?>"
      class="dropdown-item  <?php echo ($noti['seen'] == 1) ? '' : 'bg-light' ?> d-flex align-items-center justify-content-start <?php echo ($isDelete) ? 'disabled' : false ?>">
