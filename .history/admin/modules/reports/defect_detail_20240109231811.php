...
$report = firstRaw("SELECT rp.po_code, rp.code_report, rp.defect_finder, rp.quantity_deliver, rp.quantity_inspect, rp.comment, rp.suggest, rp.user_id, rp.create_at, rp.status, rs.userXX, rs.userQD, rs.userPD, rs.userGC, rs.sign_text_GC, f.name AS factory_name, p.name AS product_name, rp.create_at FROM reports AS rp JOIN factories AS f ON f.id = rp.factory_id JOIN products AS p ON p.id = rp.product_id JOIN report_sign AS rs ON report_id = rp.id WHERE rp.id = $reportId");
...
<div class="container content-pdf">
   ...
   <div class="form-group">
      ...
      <select name="status" id="chang_status" class="form-control">
         <option value="1" <?php echo ($report['status'] == 1) ? 'checked' : false ?>>Chưa duyệt</option>
         <option value="2" <?php echo ($report['status'] == 1) ? 'checked' : false ?>>Chấp nhận</option>
         <option value="3" <?php echo ($report['status'] == 1) ? 'checked' : false ?>>Trả sửa</option>
         <option value="4" <?php echo ($report['status'] == 1) ? 'checked' : false ?>>Nhận tiền trừ</option>
      </select>
   </div>
   <div class="form-group">
      <label for="suggest">Hướng xử lý (Đối với trường hợp không đạt)</label>
      <textarea name="suggest" id="add_suggest" class="form-control">
      <?php echo !empty($report['suggest']) ? $report['suggest'] : false ?>
      </textarea>
   </div>
   <div>
      <button type="button" class="btn btn-infor" id="btnAddSuggest">Xác nhận</button>
      <button type="button" class="btn btn-secondary" id="btnRefreshSuggest">Làm mới</button>
   </div>
   <?php endif; endif?>
   <br>
</div>
