...
// echo "</pre>";
 ...
 if(isPost()) {
   ...
   if(empty($errors)) {
      $arrDefectQuantity = $body["defect_quantity"];

      foreach($listAllReportDefects as $key=>$rd) {
         $listAllReportDefects[$key]['defect_quantity'] = $arrDefectQuantity[$key];
      }
      ...
      if($userXXSign->user_id == $userXXId) {
         ...
         $dataUpdateReportNoti["userXX"] = '{"user_id":'.$userXXId.', "seen":2, "sign":2, "create_at":"'.date("d-m-Y H:i:s").'"}';
      }
      if($userQDSign->user_id == $userQDId) {
         ...
         $dataUpdateReportNoti["userQD"] = '{"user_id":'.$userQDId.', "seen":2, "sign":2, "create_at":"'.date("d-m-Y H:i:s").'"}';
      }
      if($userPDSign->user_id == $userPDId) {
         ...
         $dataUpdateReportNoti["userPD"] = '{"user_id":'.$userPDId.', "seen":2, "sign":2, "create_at":"'.date("d-m-Y H:i:s").'"}';
      }
      ...
      if(!empty($listAllReportDefectFinal)) {
         echo '<pre>';
         print_r(      if(!empty($listAllReportDefectFinal)) {
         );
         echo '</pre>';
         foreach($listAllReportDefectFinal as $itemUpdate) {
            ...
            $statusUpdateReportDefect = update("report_defect", $dataUpdateReportDefect, "report_id = ".$itemUpdate["report_id"]." AND defect_id = ".$itemUpdate['defect_id']);
