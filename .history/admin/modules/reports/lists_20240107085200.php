            ...
            <?php 
               if(!empty($listReportOnPage)):
                  ...
                  foreach($listReportOnPage as $report):
                     $count++;
                     $status = null;
                     if($report['user_id'] == $user_id && !empty($signText)) {
                        $status = 1;
                     } else if(empty($signText)) {
                        $status = 3;
                     } else {
            ...
            <tr>
               ...
               <td>
                  ...
                  if($status == 1) {
                     ...
                     '<a href="'.getLinkAdmin('users', 'sign').'" class="btn btn-warning">Tạo chữ ký</a>'
                  }
               echo $status == 1 ? '<span class="btn btn-success">Đã ký</span>' : '<a class="btn btn-danger" href="'.getLinkAdmin('users', 'quick_sign', ["report_id" => $report['id'], "page" => $page]).'">Ký ngay</a>'?>
               </td>
