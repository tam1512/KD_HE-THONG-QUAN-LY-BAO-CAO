...
if(isGet()) {
   ...
   if(!empty($reportId)) {
      $defaultReport = firstRaw("SELECT rp.factory_id, rp.product_id, rp.status, f.name, p.name, po_code, conclusion, code_report, defect_finder, quantity_deliver, comment, suggest, rp.create_at FROM reports AS rp JOIN report_categories AS rp_c ON rp.cate_id = rp_c.id JOIN users AS u ON rp.user_id = u.id JOIN factories AS f ON rp.factory_id = f.id JOIN products AS p ON rp.product_id = p.id JOIN resultaql AS ra ON rp.id = ra.report_id WHERE rp.id = $reportId");;
