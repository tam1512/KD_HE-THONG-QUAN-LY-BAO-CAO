...
if(isGet()) {
   ...
   if(!empty($groupId)) {
      $defaultGroup = firstRaw("SELECT name, desc FROM groups WHERE id = '$groupId'");
 ...
 if(isPost()) {
   ...
   if(empty($desc)) {
      $errors['desc']['required'] = 'Phân quyền không được để trống';
   }
         ...
         <form action="" method="post">
            <div class="row">
               <div class="col">
                  ...
                  <div class="form-group">
                     <label for="desc">Phân quyền</label>
                     <input type="text" id="desc" name="desc" class="form-control" placeholder="Phân quyền..."
                        value="<?php echo old('desc', $old) ?>">
                     <?php echo form_error('desc', $errors, '<span class="error">', '</span>') ?>
                  </div>
