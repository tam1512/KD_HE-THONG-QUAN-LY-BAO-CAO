...
$signText = firstRaw("SELECT sign_text FROM sign WHERE user_id = $userId");
 ...
 if(isPost()) {
  ...
  if(!empty($sign) && !empty($fullname)) {
    $reportSign = firstRaw("SELECT userGC, sign_text_GC FROM report_sign WHERE id = $reportId");
    ...
    $statusUpdate = update('report_sign', $dataUpdateSign, "id = $reportId");
    if($statusUpdate) {
 ...
 if(empty($errors)) {
   $reportSign = firstRaw("SELECT * FROM report_sign WHERE id = $reportId");
   $notiReport = firstRaw("SELECT * FROM notifications WHERE id = $reportId"); 
   ...
   update('notifications', $dataUpdateNoti, "id = $reportId");
