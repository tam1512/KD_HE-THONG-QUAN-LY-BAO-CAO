...
$signText = firstRaw("SELECT sign_text FROM sign WHERE user_id = $userId");
 ...
 if(!empty($sign) && !empty($fullname)) {
  $reportSign = firstRaw("SELECT userGC FROM report_sign WHERE report_id = $reportId");
  $userGC = json_decode($reportSign['userGC'], true);
  $userGC['fullname'] = $fullname;
  $userGC['sign_at'] = date('Y-m-d H:i:s');
  $userGC['sign_text'] = $sign;
  $dataUpdate = [
    "userGC" => $userGC
  ]; 
  ...
  if($statusUpdate) {
    echo "Chỉnh sửa chữ ký sơ sở gia công thành công";
  } else {
    echo "Chỉnh sửa chữ ký sơ sở gia công thất bại";
  }
  return;
 }
