...
if($isRoot) {
 ...
 if(!$checkPermission) {
   setFlashData('msg', 'Bạn không có quyền truy cập vào trang Quản Lý Người Dùng');
...
$listAllUsers = getRaw("SELECT id, fullname, email, phone, address, group_id FROM users ORDER BY create_at DESC");
...
// Số lượng user
$countRowUsers = getRows("SELECT id FROM users $filter");
...
<form action="" method="get">
   <div class="row mb-5">
      ...
      <div class="col-12 col-sm-12 col-md-6 col-lg-2">
         <div class="form-group">
            <select name="groups" class="form-control selectpicker" data-live-search="true" data-title="Nhóm người dùng"
               data-width="100%">
               <?php 
...
<nav aria-label="Page navigation users" class="<?php echo ($numPage == 1 || empty($numPage)) ? 'd-none' : false ?>">
   <ul class="pagination pagination-sm justify-content-end">
      <li class="page-item <?php echo ($page <= 1) ? 'disabled' : '' ?>">
         <a class="page-link" href="
            <?php     
               ...
               echo getLinkAdmin('users').$queryStr.'page='.$prevPage;
            ?>">
            Trước
         </a>
      </li>
      ...
      <?php 
         if(!empty($numPage)) {
            ...
            if($numPage <= $limitPagination) {
               for($i = 1; $i <= $numPage; $i++) {
                  if($page == $i) {
                     echo '<li class="page-item active"><a class="page-link" href="'.getLinkAdmin('users').$queryStr.'page='.$i.'">'.$i.'</a></li>';
                  } else {
                     echo '<li class="page-item"><a class="page-link" href="'.getLinkAdmin('users').$queryStr.'page='.$i.'">'.$i.'</a></li>';
                  }
               }
            } else {
               for($i = $begin; $i <= $end; $i++) {
      ...
      <li class="page-item">
         ...
         <?php 
            ...
            echo getLinkAdmin('users').$queryStr.'page='.$nextPage;
         ?>">
            Sau
         </a>
      </li>
      <li class="page-item">
         ...
         <?php 
               echo getLinkAdmin('users').$queryStr.'page='.$numPage;
            ?>">
            Trang cuối
         </a>
      </li>
