...
if (
  ...
  let url =
    `${rootUrlAdmin}?module=reports&action=create_code_report";
...
if (defectElement != null && levelElement != null) {
  if (idDefectOrder != null) {
    ...
    let url =
      `${rootUrlAdmin}?module=reports&action=get_level";
    ...
    if (listTrTable != null && listTrTable.length > 0) {
      listTrTable.forEach((item) => {
        ...
        inputNumber.addEventListener("change", () => {
          ...
          let url =
            `${rootUrlAdmin}?module=reports&action=chang_defect_quantity_input";
...
//sign
$(function () {
  if (document.querySelector("#signnature") != null) {
...
$(function () {
  if (document.querySelector("#sign_GC") != null) {
    ...
    $("#disable_GC").click(function (e) {
      ...
      if (disable) {
        ...
        if (validateSignGC()) {
          ...
          let link = `${rootUrlAdmin}?module=users&action=quick_sign&report_id=${reportIdSign}`;
...
function renderChart(condition, first = false) {
  ...
  let url =
    `${rootUrlAdmin}?module=statistical_reports&action=get_data_chart";
  $.ajax({
    url: url,
...
if (btnAddSuggest && addSuggest && changeStatus) {
  btnAddSuggest.addEventListener("click", (e) => {
    ...
    let url =
      `${rootUrlAdmin}?module=reports&action=change_suggest_status";
    let isContinue = true;
      ...
      item.addEventListener("click", () => {
        ...
        CKFinder.popup({
          ...
          onInit: function (finder) {
            finder.on("files:choose", function (evt) {
              let fileUrl =
                "http://localhost" + evt.data.files.first().getUrl();
              //Xử lý chèn link ảnh vào input
              imageLink.value = fileUrl;
            });
            finder.on("file:choose:resizedImage", function (evt) {
              let fileUrl = "http://localhost" + evt.data.resizedUrl;
              //Xử lý chèn link ảnh vào input
              imageLink.value = fileUrl;
            });
          },
