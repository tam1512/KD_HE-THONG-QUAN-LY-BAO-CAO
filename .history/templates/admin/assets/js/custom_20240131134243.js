...
function AQL(quantityDeliver) {
  ...
  if (quantityDeliver == undefined) {
    simpleSize = 0;
    majorDefects = 0;
    minorDefects = 0;
  } else if (quantityDeliver >= 2 && quantityDeliver <= 8) {
    simpleSize = 2;
    majorDefects = 0;
    minorDefects = 0;
      ...
      success: function (data) {
        ...
        for (let index in data) {
          ...
          html += `<tr>
            ...
            <td>
              <a class="btn btn-danger" href="${rootUrlAdmin}bien-ban/xoa-loi-bien-ban/id=${reportId}?key=${index}" onclick="confirm('Bạn có chắc chắn muốn xóa lỗi này ?')">
