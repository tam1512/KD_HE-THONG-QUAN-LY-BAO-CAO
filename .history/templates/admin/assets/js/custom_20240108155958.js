...
function renderChart(condition, first = false) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      let list = dataChart["list"];
      delete dataChart.list;
      let labels = [];
      let data = {};
      ...
      list.forEach((item) => {
        ...
        for (var i in dataChart) {
          if (condition["valueObj"] == "all") {
            item.total_deliver = dataChart[i].total_deliver;
            item.total_inspect = dataChart[i].total_inspect;
            item.total_defect = dataChart[i].total_defect;
          }

          if (item.id === dataChart[i].id) {
            let percent =
              (dataChart[i].total_defect / dataChart[i].total_inspect) * 100;
      ...
      if (condition["valueObj"] == "all") {
        let dataTotalInspect = [];
        let dataTotalDefect = [];
        let dataPercent = [];
        list.forEach((item) => {
          dataTotalInspect.push(item.total_inspect);
          dataTotalDefect.push(item.total_defect);
        ...
        list.forEach((item) => {
          dataAlone.push(item.total_defect);
        });
        ...
        data = {
          ...
          datasets: [
            ...
            },
            {
              ...
              data: [
                18000, 10200, 6000, 9000, 10200, 3000, 9000, 10000, 10030,
                10500, 10600, 5000,
              ],
              backgroundColor: ["rgba(96, 181, 239, 0.2)"],
              borderColor: ["rgba(96, 181, 239, 1)"],
              borderWidth: 1,
              type: "bar",
            },
            {
              label: "Phần trăm lỗi",
              data: [10, 20, 60, 90, 20, 30, 90, 10, 30, 50, 60, 50],
