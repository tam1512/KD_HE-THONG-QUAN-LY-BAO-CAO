...
$(function () {
  ...
  $("#disable_GC").click(function (e) {
    ...
    if (validateSignGC()) {
      ...
      let signValue = $("#sign-text_GC").val();
      ...
      if (signTextVal) {
        ...
        console.log(dateTime); // Kết quả: 29-11-2021 15:30:45

        let html = `
        <img src="${signValue}" alt="" class="sign"><p class="d-block text-center mt-2">${fullname}</p><p class="d-block text-center">'.getDateFormat($sign_at_GC, 'd-m-Y H:i:s').'</p>' 
        `;
        document.getElementById("sign_GC").innerHTML = ``;
      }

      //lưu fullname, sign GC vào csdl
      let reportIdSign = document.getElementById("report_id").value;
      let link = `http://localhost/KimDuc/radix/admin/?module=users&action=quick_sign&report_id=${reportIdSign}&fullname=${fullname}&sign=${signValue}`;
      $.ajax({
        url: link,
        method: "GET",
        success: function (response) {
