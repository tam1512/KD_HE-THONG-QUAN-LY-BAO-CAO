...
$(function () {
  ...
  $("#disable_GC").click(function (e) {
    e.preventDefault();
    console.log(errorSign, valueSign, errorFullname, valueFullname);
    console.log(
      validateSignGC(errorSign, valueSign, errorFullname, valueFullname)
    );
    if (validateSignGC(errorSign, valueSign, errorFullname, valueFullname)) {
      ...
      let link = `http://localhost/KimDuc/radix/admin/?module=users&action=quick_sign&report_id=${reportId}&fullname=${valueFullname}&sign=${valueSign}`;
      $.ajax({
        url: link,
        method: "GET",
        success: function (response) {
...
function validateSignGC(errorSign, valueSign, errorFullname, valueFullname) {
  let check = true;
